...
export default function Comentarios({id}) {
   ...
   async function enviarComentario(formData) {        
        const datosComentarios = `${user.username}: ` + JSON.stringify(formData.comentario) 
        console.log(datosComentarios);
        try {
            await axios.post( `http://localhost:8800/api/films/${id}/agregarcomentario/`, datosComentarios )
        } catch (error) {
            console.log(error);
        }
    }
        ...
        <>
            ...
            <section className='formularios-comentarios'>
                ...
                <form onSubmit={handleSubmit(enviarComentario)}>
                    ...
                    {errors.comentario && (<span className="mensaje-error">{errors.comentario.message}</span>)}
